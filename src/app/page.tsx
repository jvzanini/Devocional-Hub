    ...
    <div className="min-h-dvh bg-[var(--color-bg)]">
      ...
      <header className="bg-[var(--color-card)]/90 backdrop-blur-md border-b border-[var(--color-border-light)] sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-5 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-light)] rounded-[var(--radius-sm)] flex items-center justify-center" style={{ boxShadow: "0 4px 12px -2px rgba(180, 83, 9, 0.20)" }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <form action={async () => { "use server"; await signOut({ redirectTo: "/login" }); }}>
              <button
                ...
                className="w-8 h-8 flex items-center justify-center text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] hover:bg-[var(--color-muted)] rounded-[var(--radius-sm)] transition-all duration-200 cursor-pointer"
              >
      ...
      <main className="max-w-2xl mx-auto px-5 py-6">
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          {[
            ...
            <div key={stat.label} className="bg-[var(--color-card)] rounded-[var(--radius-md)] border border-[var(--color-border)] p-4 text-center" style={{ boxShadow: "var(--shadow-sm)" }}>
              ...
              <div className="text-[11px] text-[var(--color-text-muted)] mt-0.5 font-medium uppercase tracking-wider">{stat.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {running > 0 && (
            <span className="inline-flex items-center gap-1.5 text-[11px] text-[var(--color-warning)] bg-[var(--color-warning-surface)] px-2.5 py-1 rounded-[var(--radius-sm)] border border-amber-200 font-medium">
              <span className="w-1.5 h-1.5 bg-[var(--color-warning)] rounded-full animate-pulse-subtle" />
        ...
        {sessions.length === 0 ? (
          <div className="text-center py-16 animate-fade-in">
            <div className="inline-flex items-center justify-center w-14 h-14 bg-[var(--color-muted)] rounded-[var(--radius-lg)] mb-4">
              <svg className="w-7 h-7 text-[var(--color-text-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
              </svg>
            </div>
